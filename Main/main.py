...
def select_image():
	...
	if (len(path) > 0):
		...
		if Infarea<5.00:
			...
			print("Disease Name- Black Spot")
			print('''Solution: The fungus overwinters in diseased canes and leaves, 
						so remove both before winter. Keep foliage clean and dry by mulching beneath plants,
						positioning roses where morning sun will quickly evaporate dew, 
						and watering at the roots rather than wetting the foliage. 
						Consider planting varieties of roses resistant to black spot.
						Plants also can be sprayed with a fungicide to prevent black spot.''')
		elif Tarea==15707.00:
			print("Disease Name-Rust")
			print('''Solution: Fungicides are available. Culturally, 
			it’s a good practice to gather and destroy any infected 
			plants to prevent the fungus from overwintering.''')
		elif Tarea==18.50:
			print("Disease Name-Black Spot")
			print('''Solution: The fungus overwinters in diseased canes and leaves, 
						so remove both before winter. Keep foliage clean and dry by mulching beneath plants,
			...
			print("Disease Name: ")
			list = ["Downey Mildew","Fusarium Wilt","Snow Mold",
					"Sooty Mold", "Anthracnose", "Cedar Rust", "BlackSpot"]
		...
		filename = directory + r"C:\Users\thaku\OneDrive\Desktop\Minor Project\Major Project\Project 1\HAM10000_metadata.csv"
		imgid = "/".join(text.split('/')[-2:])

		while True:
			if n == ord('y' or 'Y'):

				fieldnames = ['lesion_id', 'image_id', 'age', 'sex', 'localization']
...
btn.pack(side="bottom", fill="both", expand="yes", padx="250", pady="250")
